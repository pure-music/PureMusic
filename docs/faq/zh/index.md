...
* 歌曲文件名: 专辑名-歌手，（**由于修改歌手名字工作量很大，可直接按专辑分类，如过没有歌手名直接默认使用专辑歌手名，如过都没有就会读取歌曲文件中的歌手名**）
...
### 2 歌曲时长为0
由于网络歌曲无法知道时长问题，为了提高性能，没有在同步网盘的下载歌曲，需要歌曲播放后才能记录歌曲时长。

### 3 网盘歌曲是全部自动下载到本地吗？
不是，只有播放的时候才会分段下载歌曲文件，并且配置了缓存，减少本地磁盘占用。
