...
private constructor(private val mContext: Context) : ThemeStorePrefKeys, ThemeStoreInterface {
    ...
    companion object {
        ...
        var themeValue: ThemeValue = object : ThemeValue {
        ...
        fun accentColor(context: Context): Int {
            return accentColor(context, themeValue)
        }

        @CheckResult
        @ColorInt
        private fun accentColor(context: Context, themeValue: ThemeValue): Int {
            // Set MD3 accent if MD3 is enabled or in-app accent otherwise
            if (themeValue.isMD3Enabled(context) && VersionUtils.hasS()) {
            ...
            val color = if (themeValue.isWallpaperAccentEnabled(context)) {
                wallpaperColor(context, isWindowBackgroundDark(context))
            } else {
                if (themeValue.isBlack(context)) {
                    return ContextCompat.getColor(context, R.color.m3_accent_color2_night)
                } else {
        ...
        fun isMD3Enabled(context: Context): Boolean {
            return themeValue.isMD3Enabled(context)
        }
...
//        }
    ...
    open interface ThemeValue {
        fun isBlack(context: Context): Boolean
        fun isMD3Enabled(context: Context): Boolean
