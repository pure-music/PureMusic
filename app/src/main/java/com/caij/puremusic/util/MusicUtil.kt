...
object MusicUtil : KoinComponent {
    ...
    fun getLyrics(context: Context, song: Song): String {
        var lyrics: String? = null
        val lrcFile = LyricUtil.getLrcFile(context, song)
        if (lrcFile != null) {
            lyrics = LyricUtil.getStringFromLrc(lrcFile)
    ...
    fun isArtistNameUnknown(artistName: String?): Boolean {
        ...
        if (artistName == BaseArtistUtil.UNKNOWN_ARTIST_DISPLAY_NAME) {
            return true
        }
        if (artistName == BaseSongUtil.DEFAULT_ARTIST) {
            return true
        }
    ...
    fun isVariousArtists(artistName: String?): Boolean {
        ...
        if (artistName == BaseArtistUtil.VARIOUS_ARTISTS_DISPLAY_NAME) {
            return true
        }
        return false
    }
    ...
    fun getArtistInfoString1(
        context: Context,
        songs: List<Song>
    ): String {
        val albumCount = BaseAlbumUtil.splitIntoAlbums(songs).size
        val albumString =
