...
import com.caij.puremusic.extensions.getIntRes
import com.caij.puremusic.extensions.getStringOrDefault
...
import com.caij.puremusic.transform.*
...
object PreferenceUtil {
    val sharedPreferences: SharedPreferences = MMKVSharedPreferencesFactory().getSharedPreferences(App.getContext(),
        App.getContext().packageName + "_preferences3.mmkv", Context.MODE_PRIVATE)

    val defaultCategories = listOf(
        CategoryInfo(CategoryInfo.Category.Home.idHolder, true),
        CategoryInfo(CategoryInfo.Category.Songs.idHolder, true),
        CategoryInfo(CategoryInfo.Category.Albums.idHolder, true),
        CategoryInfo(CategoryInfo.Category.Artists.idHolder, true),
        CategoryInfo(CategoryInfo.Category.Playlists.idHolder, true),
    )

    val defaultItemCategories = listOf(
    ...
    @JvmStatic
    fun findCategory(idHolder: Int): CategoryInfo.Category? {
        for (item in defaultItemCategories) {
            if (idHolder == item.idHolder) {
                return item
            }
        }
        return null
...
//    var musicSourceFolder: List<Folder>
//        get() {
//            val gson = Gson()
//            val collectionType = object : TypeToken<List<Folder>>() {}.type
//
//            val data = sharedPreferences.getStringOrDefault(
//                MUSIC_SOURCE_FOLDER,
//                gson.toJson(defaultFolders, collectionType)
//            )
//            return try {
//                Gson().fromJson(data, collectionType)
//            } catch (e: JsonSyntaxException) {
...
//            val collectionType = object : TypeToken<List<Folder?>?>() {}.type
//            sharedPreferences.edit {
//                putString(MUSIC_SOURCE_FOLDER, Gson().toJson(value, collectionType))
//            }
//        }
    ...
    val baseTheme get() = sharedPreferences.getStringOrDefault(GENERAL_THEME, "light")

    fun getGeneralThemeValue(isSystemDark: Boolean): ThemeMode {
        val themeMode: String =
            sharedPreferences.getStringOrDefault(GENERAL_THEME, "light")
    ...
    private val autoDownloadSongImagesPolicy
        get() = sharedPreferences.getStringOrDefault(
            AUTO_DOWNLOAD_SONG_IMAGES_POLICY,
            "always"
        )
...
//        )
    ...
    fun isAllowedToDownloadArtistCover(context: Context): Boolean {
        return isAllowedToDownloadCover(context, autoDownloadImagesPolicy)
    }


    fun isAllowedToDownloadSongCover(context: Context): Boolean {
        return isAllowedToDownloadCover(context, autoDownloadSongImagesPolicy)
    }

    private fun isAllowedToDownloadCover(context: Context, policy: String): Boolean {
        return when (policy) {
            "always" -> true
//            "only_wifi" -> {
...
//            }
    ...
    var artistGridStyle: GridStyle
        get() {
            val id: Int = sharedPreferences.getInt(ARTIST_GRID_STYLE, 0)
            return GridStyle.values().firstOrNull { gridStyle ->
                gridStyle.id == id
            } ?: GridStyle.Circular
        }
    ...
    var startDirectory: File
        get() {
            val folderPath = FoldersSettingFragment.defaultStartDirectory.path
    ...
    val statusBarLyric
        get() = sharedPreferences.getBoolean("status_bar_lyric", true)
}
