...
import com.caij.puremusic.helper.SortOrder.AlbumSortOrder
...
class AlbumsFragment : AbsRecyclerViewCustomGridSizeFragment<AlbumAdapter, GridLayoutManager>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        loadData()
        EventBus.register(EVENT_ALBUMS_UPDATE, this)
    }

    private fun loadData() {
        lifecycleScope.launch(Dispatchers.IO) {
            var albums = libraryViewModel.fetchAlbums()
            albums = AlbumUtil.sort(albums)
            withContext(Dispatchers.Main) {
                for (album in albums) {
                    AlbumDetailsFragment.cache[album.albumId] = album
                }
    ...
    override fun onShuffleClicked() {
        lifecycleScope.launch(Dispatchers.IO) {
            val it = libraryViewModel.fetchAlbums()
            val songs = it.shuffled().flatMap { album ->
                libraryViewModel.getAlbumSongs(album.id)
            }
            withContext(Dispatchers.Main) {
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        EventBus.unregister(EVENT_ALBUMS_UPDATE, this)
    }

    override fun onEvent(key: String, value: Any?) {
        if (key == EVENT_ALBUMS_UPDATE) {
            loadData()
        }
    }
}
