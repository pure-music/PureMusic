...
import com.caij.puremusic.helper.SortOrder.SongSortOrder
...
class SongsFragment : AbsRecyclerViewCustomGridSizeFragment<SongAdapter, GridLayoutManager>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        loadData()
        EventBus.register(EVENT_SONGS_UPDATE, this)
        EventBus.register(EVENT_SONG_UPDATE, this)
    }

    private fun loadData() {
        lifecycleScope.launch(Dispatchers.IO) {
            val it = libraryViewModel.fetchSongs()
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        EventBus.unregister(EVENT_SONGS_UPDATE, this)
    ...
    override fun onEvent(key: String, value: Any?) {
        if (key == EVENT_SONGS_UPDATE) {
            loadData()
        } else if (key == EVENT_SONG_UPDATE && value is Song) {
            if (adapter != null) {
                for (i in 0 until adapter!!.dataSet.size) {
                    val song = adapter!!.dataSet[i]
                    if (song.id == value.id) {
                        adapter!!.dataSet[i] = value
