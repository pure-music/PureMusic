...
import com.caij.puremusic.databinding.FragmentMiniPlayerBinding
...
open class MiniPlayerFragment : AbsMusicServiceFragment(R.layout.fragment_mini_player),
    MusicProgressViewUpdateHelper.Callback, View.OnClickListener, EventObserver {
    ...
    private fun setUpMiniPlayer() {
        ...
        val color = ThemeStore.accentColor(requireActivity())
        binding.progressBar.setIndicatorColor(color)
        binding.progressBar.trackColor = ColorUtil.withAlpha(color, 0.1f)

        binding.secondaryProgressBar.setIndicatorColor(ColorUtil.withAlpha(color, 0.15f))
    }
...
//        binding.text.text = song.artistName
    ...
    override fun onUpdateProgressViews(progress: Int, bufferedPosition: Int, total: Int) {
        ...
        binding.secondaryProgressBar.progress = bufferedPosition
        binding.secondaryProgressBar.max = total
    }
    ...
    override fun onEvent(key: String, value: Any?) {
        val song = MusicPlayerRemote.currentSong
        if (value is Song && value.id == song.id) {
            updateSongTitle()
            updateSongCover()
        }
    }
