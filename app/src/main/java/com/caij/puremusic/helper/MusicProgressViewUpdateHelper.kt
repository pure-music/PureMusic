...
class MusicProgressViewUpdateHelper : Handler {
    ...
    private fun refreshProgressViews(): Int {
        ...
        val bufferedPosition = MusicPlayerRemote.bufferedPosition
        if (totalMillis > 0) {
            firstUpdate = false
            callback?.onUpdateProgressViews(progressMillis, bufferedPosition, totalMillis)
        } else {
            // 默认holder值
            callback?.onUpdateProgressViews(0, 0, 100)
        }
    ...
    interface Callback {
        fun onUpdateProgressViews(progress: Int, bufferedPosition: Int, total: Int)
    }
