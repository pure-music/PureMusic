...
class CoverLrcView @JvmOverloads constructor(
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        mPlayDrawable =
            if (mPlayDrawable == null) ContextCompat.getDrawable(
                context,
                R.drawable.ic_play_arrow_accent
            ) else mPlayDrawable
    ...
    fun setTimelineColor(timelineColor: Int) {
        ...
        mPlayDrawable?.setTint(timelineColor)
        postInvalidate()
    }
    ...
    fun loadLrc(lrcEntries: List<LrcEntry>?) {
...
//    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isShowTimeline) {
            ...
            var realTimeTextWidth = mTimePaint.measureText(timeText)
            if (realTimeTextWidth <= 0) {
                realTimeTextWidth = mTimeTextWidth.toFloat()
            }
            val timeX = width - realTimeTextWidth / 2
    ...
    private fun onLrcLoaded(entryList: List<LrcEntry>?) {
        ...
        if (!entryList.isNullOrEmpty()) {
            for (entry in entryList) {
                mLrcEntryList.add(LrcEntry(entry.time, entry.text, entry.secondText))
            }
        }
        initEntryList()
        invalidate()
